...
class CollectionTable extends PureComponent {
  ...
  onSearch = () => {
    ...
    this.setState({
      ...
      list: list
        .map(record => {
          const match = record.newsInfo.nTitle.match(reg);
          if (!match) {
            return null;
          }
          return {
            ...
            'newsInfo.nTitle': (
              <span>
                {record.newsInfo.nTitle.split(reg).map(
                  (text, i) =>
                    i > 0
                      ? [
                          <span key={record.cID} className={styles.highlight}>
                            {match[0]}
                          </span>,
                          text
                        ]
